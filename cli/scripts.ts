...
export const lockToken = async (
    ...
    try {
        const tx = await createLockTokenTx(payer.publicKey, program, solConnection, 2);
        const txId = await provider.sendAndConfirm(tx, [], {
            commitment: "confirmed",
        });
...
export const lockSol = async (
    ...
    try {
        const tx = await createLockSolTx(payer.publicKey, program, 0.1);
        const txId = await provider.sendAndConfirm(tx, [], {
            commitment: "confirmed",
        });
...
export const claim = async (
    ...
    try {
        const tx = await createClaimTx(payer.publicKey, program, solclaim);
